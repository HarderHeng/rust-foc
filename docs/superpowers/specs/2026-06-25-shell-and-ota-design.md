...
- shell `ota_update` 命令触发:写 flag → defmt 提示 → sys_reset
...
**为什么拆 workspace:** bootloader 和 app 是两个独立的 ELF 二进制,不能塞进同一 crate。app 编译产物是 `target/thumbv7em-none-eabihf/debug/foc-rust.elf`,bootloader 是 `target/.../bootloader.elf`,各自 link 各自的 `memory.x`。
...
0x0800_0000  +------------------+ 0 KB
              ...
              |  代码            |
0x0800_35F0  +------------------+
              |  config page     |  2 KB  (1 page, 2KB on G431)
              |  0x0800_3F00:    |        ↑ flag 字节
...
`APP_START_ADDRESS = 0x0800_4000` (常量,两个 crate 都用)。
...
ota.rs::OtaUpdateCommand::run(ctx):
  ...
  2. 短暂 delay 50ms,让 UART 把上一步字节真正发完(避免与 reset 竞态)
  3. 写 OTA_FLAG = 0xAA
...
- 强约束:**bootloader 总占用 ≤ 14 KB**,否则挤占 app
- 验证手段:`cargo size` 每次改 bootloader 后必跑
